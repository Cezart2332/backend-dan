      ...
      <LinearGradient colors={['#f0f8ff', '#e6f3ff', '#ffffff']} style={styles.gradient}>
        <ScrollView contentContainerStyle={styles.content}>
          ...
          <Text style={styles.subtitle}>
            Alege categoria potrivită și vizionează introducerea video înainte de a accesa audio-urile.
          </Text>
          {options.map((item) => (
            <TouchableOpacity
              key={item.id}
              style={styles.card}
              onPress={() => navigation.navigate('IntelegeAnxietateVideo', {
                title: item.title,
...
const styles = StyleSheet.create({
  ...
  cardTitle: { fontSize: 16, fontWeight: '600', color: '#2c3e50', marginBottom: 6 },
  cardSubtitle: { fontSize: 13, color: '#6c7b84', marginBottom: 10, lineHeight: 18 },
  arrow: { fontSize: 18, color: '#4a90e2', fontWeight: '700', textAlign: 'right' },
  backBtn: {
    alignSelf: 'center',
    marginTop: 20,
